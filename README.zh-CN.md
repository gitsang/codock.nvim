...
- `src/main.lua:42-50` - 行范围，跳转到第 42 行
- `src/main.lua#L42` - GitHub 风格的行号链接
- `src/main.lua:42:7:some text` - `grep`/ripgrep 的匹配输出，后面的文本会被忽略
- `b/src/main.lua` - git diff 的路径，`a/` 和 `b/` 前缀会被自动去掉

路径可以被引号、反引号、圆括号、方括号或尖括号包裹；路径后面的标点（如 `see src/main.lua.`）不会被视为文件名的一部分。绝对路径和 `~/` 路径同样可用，`@` 前缀（`:CodockFilePosYank` 会生成）会被忽略。

相对路径会优先相对于终端工作目录解析，其次相对于 Neovim 的工作目录解析，因此即使 CLI 切换过目录，它输出的路径也能正确解析。

> 点击的文本必须对应一个**真实存在**的文件。CLI 只输出裸文件名（例如真实位置是 `lua/codock/utils.lua` 但只打印了 `utils.lua`）、相对于 CLI 知道而 Neovim 不知道的目录的路径、或指向目录的路径，都不会被打开。文件名中含有与正文常用符号相同字符（空格、括号、逗号、`#` 等）的情况也能识别：只有当更宽的候选字符串确实对应存在于磁盘上的文件时才会被采纳。

只有当光标下的文本能解析为已存在的文件时才会处理点击；其他点击保持原有行为，包括文本选择和把点击事件透传给 CLI。
...
如果希望点击的文件单独占一个分屏（这样正在编辑的文件会与它并排显示），可以设置：
