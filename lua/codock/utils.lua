...
---Find a normal window that already displays a buffer.
---
---Floating windows are ignored, so a temporary popup showing the same file is
---never mistaken for a real editing window.
---@param buf integer buffer to look for
---@return integer|nil win
function M.find_window_showing(buf)
	for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
		if vim.api.nvim_win_is_valid(win) and vim.api.nvim_win_get_config(win).relative == "" then
...
---Open a file in a new vertical split next to an editor window.
---
---A new window is created instead of replacing the buffer of an existing one,
---so the file that was being edited stays open. `nvim_open_win(..., false, ...)`
---leaves the focused window - the terminal the click came from - untouched, so
---the CLI session keeps receiving input and stays in terminal mode.
---
---The codock terminal keeps its configured width because it sets
---`winfixwidth`. `split` names the side of the anchor the new window goes on,
---so the file ends up on the side facing the terminal.
---@param terminal_win integer|nil window the click came from
---@param anchor_win integer|nil editor window to split
---@param abs_path string absolute path of an existing file
---@param line integer|nil 1-based line to jump to
---@param col integer|nil 1-based byte column to jump to
---@return integer|nil win the new window, or nil when it could not be opened
function M.open_file_in_split(terminal_win, anchor_win, abs_path, line, col)
	local buf = load_file_buffer(abs_path)
	if not buf then
		return nil
	end

	-- A file that is already on screen is reused instead of opening a second
	-- view of it, so clicking the same path twice does not split twice.
	local existing = M.find_window_showing(buf)
	if existing then
		move_cursor_in(existing, buf, line, col)
		return existing
	end
	...
	-- Place the new window on the side of the anchor that faces the terminal,
	-- so the opened file sits next to the terminal the click came from.
	local side = "right"
	if terminal_win and vim.api.nvim_win_is_valid(terminal_win) then
		local anchor_col = vim.api.nvim_win_get_position(anchor_win)[2]
		local terminal_col = vim.api.nvim_win_get_position(terminal_win)[2]
		if terminal_col < anchor_col then
			side = "left"
		end
	end
